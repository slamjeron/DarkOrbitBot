...
public class textReader{
	...
	public void start(BufferedImage image, String shipName){
	...
	if(!cltest(im,rt.petHPPt,Color.green)&&!cltest(im,rt.petHPPt,Color.black)){
	...
	public class displayANCalkinfo{
		...
		DateFormat dateTM = new SimpleDateFormat("yyyy/MM/dd HH:mm:ss");
		DateFormat date = new SimpleDateFormat("yyyy-MM-dd");
		Date today = new Date();
		File cwdFile = new File (".");
		String cwd = cwdFile.getAbsolutePath();
		private File myFile= new File(shipName+date.format(today)+ ".txt");
		public void calk(int min) {
			...
			boolean st = min==0;
			boolean cnt400=(min%500!=0);
			if(st^cnt400){
				if(da1){
				
				String todayst = dateTM.format(today);
				saveStr+=todayst+"\r \r";
				da1=false;
				}
			}else{
	...
	private void saveUI(){
	 ...
	 public String getProgramPath2() throws UnsupportedEncodingException {
	      URL url = textReader.class.getProtectionDomain().getCodeSource().getLocation();
	      String jarPath = URLDecoder.decode(url.getFile(), "UTF-8");
	      String parentPath = new File(jarPath).getParentFile().getPath();
	      //parentPath=new File(parentPath).getParentFile().getPath();
	      return parentPath;
	   }
	 ...
	 public void makeDir(){
		 try {
	         ...
	         String newDir = path + fileSeparator + shipName + fileSeparator;
		...
		public void openUser(File file) {
			...
			myFile= new File(file.getPath()+"\\"+date.format(today)+ ".txt");
